...
func TestGetJobsByCommit(t *testing.T) {
	// Create a new request with a mock commit SHA
	...
	r := mux.NewRouter()
	ch := &commitHandler{} // Assuming CommitHandler is the handler for GetJobsByCommit
	r.HandleFunc("/v1/{owner}/{repo}/commit/getJobsByCommit", ch.GetJobsByCommit).Methods("GET")

	// Create a response recorder to record the response
	rr := httptest.NewRecorder()

	// Serve the request to the registered handler
	r.ServeHTTP(rr, req)
